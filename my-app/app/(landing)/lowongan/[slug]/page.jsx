...
import {
  ...
  CalendarDays, ArrowLeft, AlertCircle, Copy, Check, Mail
} from "lucide-react";
...
const relatedJobs = [
  { id: 2, title: "UI/UX Designer",    type: "FULL-TIME", salary: "7000000-10000000",  skills: ["Figma", "Sketch", "Prototyping"],   company: "Tokopedia",        location: "Jakarta, Indonesia", logo: "T"  },
  { id: 3, title: "Backend Developer", type: "FULL-TIME", salary: "10000000-15000000", skills: ["Node.Js", "Express", "PostgreSQL"],  company: "Gojek",            location: "Jakarta, Indonesia", logo: "GJ" },
  { id: 4, title: "Mobile Developer",  type: "FULL-TIME", salary: "9000000-14000000",  skills: ["Flutter", "React Native", "Swift"],  company: "Shopee Indonesia",  location: "Jakarta, Indonesia", logo: "S"  },
];
...
function RelatedJobCard({ job }) {
  return (
    <Link
      href={`/lowongan/${job.slug || job.id}`}
      className="p-4 border border-slate-100 rounded-xl hover:border-[#6CC1FF] hover:bg-blue-50/30 transition-all duration-150 cursor-pointer group block"
    >
      <h4 className="text-[13.5px] font-bold text-slate-800 mb-1.5 group-hover:text-[#0A66C2] transition-colors line-clamp-2">
        {job.title}
      </h4>
      <div className="flex items-center gap-2 mb-2.5 flex-wrap">
        <span className="text-[10px] font-bold px-2 py-0.5 rounded-sm border tracking-wide bg-[#EFF6FF] text-[#1D4ED8] border-[#93C5FD]">{job.type}</span>
        <span className="text-[11.5px] text-sky-700 font-semibold">{formatRupiah(job.salary)}</span>
      </div>
      <div className="flex flex-wrap gap-1.5 mb-3">
        {job.skills.map((s) => (
          <span key={s} className="text-[10.5px] font-semibold px-2.5 py-0.5 rounded-md border bg-[#F8FAFC] text-[#334155] border-[#CBD5E1]">{s}</span>
        ))}
      </div>
      <div className="flex items-center gap-2">
        <CompanyLogo logo={job.logo} size="sm" />
        <div className="min-w-0">
          <p className="text-[12px] font-semibold text-slate-700 leading-tight truncate">{job.company}</p>
...
function LoadingSkeleton() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-white pt-5">
      <div className="bg-white border-b border-slate-100 px-8 pb-4 pt-20">
        <div className="max-w-[1200px] mx-auto"><Skeleton className="h-4 w-48" /></div>
      </div>
...
export default function JobDetailPage() {
  ...
  useEffect(() => {
    // ✅ ganti dari params.id menjadi params.slug
    ...
    const fetchDetail = async () => {
      try {
        ...
        setJob({
          id:               item.id,
          slug:             item.slug, // ✅ simpan slug untuk dipakai di link internal
          title:            item.posisi,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-white font-[Plus_Jakarta_Sans,Segoe_UI,sans-serif] pt-5">
      <div className="bg-white border-b border-slate-100 px-8 pb-4 pt-20">
        <div className="max-w-[1200px] mx-auto flex items-center gap-2 text-[12.5px] text-slate-400">
          <Link href="/" className="hover:text-[#0A66C2] transition-colors">Beranda</Link>
          <ChevronRight size={13} />
          <Link href="/lowongan" className="hover:text-[#0A66C2] transition-colors">Lowongan</Link>
          <ChevronRight size={13} />
            ...
            <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between gap-3 pt-4 border-t border-slate-100 flex-wrap">
                  ...
                  <button
                    onClick={() => router.push(`/lowongan/${job.slug}/lamar`)}
