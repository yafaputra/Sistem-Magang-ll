...
// ─── Fonts — konsisten dengan Kelola Lowongan ─────────────────────────────────
const FONTS = `
...
// ─── Status Config (disederhanakan, warna flat) ───────────────────────────────
const STATUS_CONFIG = {
  PENDING_BERKAS:        { label: "Menunggu Berkas",  dot: "bg-slate-300",   badge: "bg-slate-100 text-slate-600 border border-slate-200" },
  BERKAS_DITERIMA:       { label: "Berkas Disetujui", dot: "bg-[#0A66C2]",   badge: "bg-[#EFF6FF] text-[#0A66C2] border border-[#93C5FD]" },
  BERKAS_DITOLAK:        { label: "Berkas Ditolak",   dot: "bg-red-400",     badge: "bg-red-50 text-red-600 border border-red-200" },
  INTERVIEW_DIJADWALKAN: { label: "Interview",        dot: "bg-violet-400",  badge: "bg-violet-50 text-violet-700 border border-violet-200" },
  LOLOS_INTERVIEW:       { label: "Lolos Interview",  dot: "bg-amber-400",   badge: "bg-amber-50 text-amber-700 border border-amber-200" },
  TIDAK_LOLOS_INTERVIEW: { label: "Tidak Lolos",      dot: "bg-red-400",     badge: "bg-red-50 text-red-600 border border-red-200" },
  DITERIMA_MAGANG:       { label: "Diterima",         dot: "bg-emerald-400", badge: "bg-emerald-50 text-emerald-700 border border-emerald-200" },
  KONFIRMASI_DITERIMA:   { label: "Terkonfirmasi",    dot: "bg-emerald-600", badge: "bg-emerald-100 text-emerald-800 border border-emerald-300" },
  DITOLAK:               { label: "Ditolak",          dot: "bg-red-400",     badge: "bg-red-50 text-red-600 border border-red-200" },
};

const STATUS_COMPAT = { pending: "PENDING_BERKAS", accepted: "DITERIMA_MAGANG", rejected: "DITOLAK" };
...
// ─── Stat Cards Config (dipakai untuk ledger strip) ────────────────────────────
const STAT_STRIP = [
  { key: "total",    label: "Total Pelamar",   icon: "users",        color: "text-slate-700" },
  { key: "menunggu", label: "Menunggu Review",  icon: "clock",        color: "text-[#0A66C2]" },
  { key: "proses",   label: "Dalam Proses",     icon: "refresh",      color: "text-violet-600" },
  { key: "diterima", label: "Diterima Magang",  icon: "check-circle", color: "text-emerald-600" },
];
...
const STEP_STYLE = {
  ...
  active:   { circle: "bg-[#EFF6FF] border-[#0A66C2] text-[#0A66C2]",       label: "text-[#0A66C2]",   line: "bg-slate-200",   icon: "●" },
      ...
      {TAHAPAN.map((t, i) => {
        ...
        return (
          <div key={t.key} className="flex items-start">
            ...
            {i < TAHAPAN.length - 1 && <div className={`w-9 h-0.5 mt-3.5 mx-0.5 flex-shrink-0 ${s.line}`} />}
          </div>
        );
      })}
    </div>
...
// ─── Interview Modal (tidak berubah secara fungsional, hanya warna disamakan) ─
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl border border-slate-200 w-full max-w-md overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div>
            <div className="text-[14px] font-bold text-slate-800 font-display">{existing ? "Edit Jadwal Interview" : "Jadwalkan Interview"}</div>
        ...
        <div className="px-5 py-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="date" value={form.tanggal} onChange={(e) => set("tanggal", e.target.value)}
                className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-[12.5px] text-slate-700 focus:outline-none focus:border-[#0A66C2]" />
            </div>
            <div>
          ...
          <div>
            ...
            <div className="relative">
              <div className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"><Icon name="link" className="w-3.5 h-3.5" /></div>
              <input type="url" placeholder="https://meet.google.com/..." value={form.linkMeeting} onChange={(e) => set("linkMeeting", e.target.value)}
                className="w-full border border-slate-200 rounded-xl pl-9 pr-3 py-2.5 text-[12.5px] text-slate-700 focus:outline-none focus:border-[#0A66C2]" />
            </div>
          </div>
        </div>

        <div className="px-5 pb-5 flex gap-2">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl text-sm font-semibold border border-slate-200 text-slate-500 hover:bg-slate-50">
            Batal
          </button>
          <button onClick={() => valid && onSave(form)} disabled={!valid}
            className="flex-1 py-2.5 rounded-xl text-sm font-semibold bg-[#0A66C2] text-white hover:bg-[#08519c] disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2">
...
// ─── Action Panel — disederhanakan (satu warna aksen: #0A66C2) ────────────────
function ActionHeader({ icon, children }) {
  return (
    <div className="flex items-center gap-2 mb-4">
      <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 border border-slate-200 bg-slate-50 text-slate-500">
...
function ActionButton({ onClick, tone, icon, children, disabled }) {
  const tones = {
    ...
    primary: "bg-[#0A66C2] text-white border-[#0A66C2] hover:bg-[#08519c]",
...
function ActionPanel({ selected, onStatusChange, onOpenInterview }) {
  ...
  if (s === "PENDING_BERKAS") {
    body = (
      <>
        <ActionHeader icon="file">Verifikasi Berkas</ActionHeader>
    ...
    body = (
      <>
        <ActionHeader icon="calendar">Jadwalkan Interview</ActionHeader>
        <ActionButton tone="primary" icon="calendar" onClick={onOpenInterview}>Jadwalkan Interview</ActionButton>
      </>
    );
    ...
    body = (
      <>
        <ActionHeader icon="calendar">Interview Dijadwalkan</ActionHeader>
        {j && (
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-2 mb-3">
            {[
              ...
              <div key={k} className="flex gap-2 items-start font-mono">
                <span className="text-[10.5px] text-slate-400 w-14 flex-shrink-0 font-semibold pt-0.5">{k}</span>
                <span className="text-[12px] text-slate-700 font-medium break-all">{v}</span>
              </div>
            ))}
          </div>
        )}
    ...
    body = (
      <>
        <ActionHeader icon="star">Hasil Seleksi</ActionHeader>
        <div className="flex gap-2">
    ...
    body = (
      <>
        <ActionHeader icon="clock">Menunggu Konfirmasi Mahasiswa</ActionHeader>
    ...
    body = (
      <>
        <ActionHeader icon="check-circle">Selesai — Mahasiswa Bergabung</ActionHeader>
        <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3">
    ...
    if (msg) {
      body = (
        <>
          <ActionHeader icon="x-circle">Status Final</ActionHeader>
  ...
  if (!body) return null;
  return <div className="bg-white border border-slate-200 rounded-2xl p-5">{body}</div>;
}
  ...
  return (
    <button onClick={onClick}
      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-all border mb-0.5
        ${isActive ? "bg-[#0A66C2] border-[#0A66C2]" : "border-transparent hover:bg-slate-50 hover:border-slate-200"}`}>
      <div className={`w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center text-[11px] font-bold border
        ${isActive ? "bg-white/15 text-white border-white/20" : `${avBg} ${avText} ${avBorder}`}`}>
        {getInitials(applicant.name)}
      </div>
      <div className="flex-1 min-w-0">
        <div className={`text-[12.5px] font-semibold truncate ${isActive ? "text-white" : "text-slate-800"}`}>{applicant.name}</div>
        <div className={`text-[11px] truncate mt-0.5 ${isActive ? "text-white/70" : "text-slate-400"}`}>{applicant.position}</div>
      </div>
  ...
  return (
    <div className="space-y-3">
      <div className="bg-white border border-slate-200 rounded-2xl p-5">
        <div className="text-[10px] font-semibold tracking-widest uppercase text-slate-400 mb-3">Keahlian</div>
        {applicant.skills.length > 0 ? (
          <div className="flex flex-wrap gap-1.5">
            {applicant.skills.map((skill) => (
              <span key={skill} className="px-3 py-1 rounded-lg bg-[#EFF6FF] text-[#0A66C2] text-[11.5px] font-medium border border-[#93C5FD]">{skill}</span>
            ))}
          </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5">
        <div className="text-[10px] font-semibold tracking-widest uppercase text-slate-400 mb-4">Detail Akademik</div>
        <div className="grid grid-cols-2 gap-x-6 gap-y-4">
          {[
            ...
            <div key={label}>
              <div className="text-[10.5px] text-slate-400 mb-0.5 font-mono">{label}</div>
...
// ─── Documents Section — ✅ 3 file: CV, Surat Lamaran, Transkrip ──────────────
function DocumentCard({ doc }) {
  ...
  return (
    <div className={`flex items-center justify-between gap-3 rounded-xl px-4 py-3 border
      ${uploaded ? "bg-white border-slate-200" : "bg-slate-50 border-dashed border-slate-200"}`}>
      <div className="flex items-center gap-3 min-w-0">
        <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 border
          ${uploaded ? "bg-[#EFF6FF] border-[#93C5FD] text-[#0A66C2]" : "bg-slate-100 border-slate-200 text-slate-300"}`}>
          <Icon name="file-text" className="w-4 h-4" />
        </div>
        <div className="min-w-0">
          <div className="text-[12.5px] font-semibold text-slate-700 flex items-center gap-1.5">
            {doc.label}
            {doc.required && <span className="text-[9px] font-bold text-red-400">*</span>}
          </div>
          <div className="text-[11px] text-slate-400 truncate">{doc.filename || "Belum diunggah"}</div>
        </div>
      </div>
      {uploaded && (
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <a href={doc.url} target="_blank" rel="noopener noreferrer"
            className="w-8 h-8 rounded-lg border border-slate-200 flex items-center justify-center text-slate-500 hover:bg-slate-50" title="Lihat">
            <Icon name="eye" className="w-3.5 h-3.5" />
          </a>
          <a href={doc.url} target="_blank" rel="noopener noreferrer" download={doc.filename}
            className="w-8 h-8 rounded-lg border border-[#93C5FD] bg-[#EFF6FF] flex items-center justify-center text-[#0A66C2] hover:bg-[#DBEAFE]" title="Download">
...
function DocumentsSection({ applicant }) {
  const docs = applicant.documents;
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5">
      <div className="text-[10px] font-semibold tracking-widest uppercase text-slate-400 mb-4">Dokumen Lamaran</div>
      <div className="flex flex-col gap-2.5">
        <DocumentCard doc={docs.cv} />
        <DocumentCard doc={docs.coverLetter} />
        <DocumentCard doc={docs.transcript} />
      </div>
    </div>
  );
}
...
export default function DaftarPelamar() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/rekrutmen/pelamar`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center h-screen bg-slate-50 gap-4">
      ...
      <button onClick={() => window.location.reload()}
        className="px-4 py-2 rounded-xl bg-[#0A66C2] text-white text-sm font-semibold hover:bg-[#08519c]">
        Coba Lagi
      </button>
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 font-sans">
      ...
      {showInterviewModal && (
        <InterviewModal onClose={() => setShowInterviewModal(false)} onSave={handleSaveInterview} existing={selected?.jadwalInterview} />
      )}
      ...
      {/* Stat strip — ledger style, sama seperti Kelola Lowongan */}
      <div className="px-6 pt-4 flex-shrink-0">
        <div className="grid grid-cols-4 max-[900px]:grid-cols-2 bg-white border border-slate-200 rounded-2xl overflow-hidden">
          {STAT_STRIP.map((s, i) => (
            <div key={s.key}
              className="px-5 py-4 flex flex-col gap-1.5 border-r border-dashed border-slate-200 last:border-r-0 max-[900px]:[&:nth-child(2)]:border-r-0">
              <div className="flex items-center gap-1.5">
                <span className={s.color}><Icon name={s.icon} className="w-3.5 h-3.5" /></span>
                <span className={`font-mono text-[10px] uppercase tracking-[0.14em] font-semibold ${s.color}`}>{s.label}</span>
              </div>
              <span className={`font-display text-[28px] font-semibold leading-none ${s.color}`}>{counts[s.key]}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden px-6 py-4 gap-4">
        ...
        <div className="w-[250px] flex-shrink-0 bg-white border border-slate-200 rounded-2xl flex flex-col overflow-hidden">
          <div className="px-4 pt-4 pb-3 border-b border-slate-100 flex-shrink-0">
            <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 focus-within:border-[#0A66C2] transition-colors">
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3">
            ...
            {filtered.length === 0 ? (
              <div className="py-10 text-center text-slate-400">
                <Icon name="search" className="w-5 h-5 text-slate-300 mx-auto mb-2" />
        ...
        {selected ? (
          <div className="flex-1 overflow-y-auto space-y-3">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[15px] font-semibold text-slate-800 font-display">{selected.name}</div>
                  <div className="flex items-center gap-1.5 text-xs text-slate-400 mt-0.5">
                    <Icon name="map-pin" className="w-3 h-3" />{selected.university}
                  </div>
                </div>
              ...
              <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-slate-100 font-mono">
                {[
                  ...
                  <div key={val} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-50 text-slate-600 text-xs border border-slate-200">
                    <Icon name={icon} className="w-3 h-3 text-slate-400" />{val}
                  </div>
                ))}
              </div>
            </div>

            {/* Tab Switcher — CV/Resume diganti jadi Dokumen (3 file) */}
            <div className="flex bg-white border border-slate-200 rounded-2xl overflow-hidden">
              {[
                { key: "info", label: "Info Pelamar", icon: "user" },
                { key: "cv",   label: "Dokumen",       icon: "clipboard" },
              ].map((tab) => (
                <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                  className={`flex-1 py-3 text-xs font-semibold flex items-center justify-center gap-2 border-none cursor-pointer transition-all
                    ${activeTab === tab.key ? "bg-[#EFF6FF] text-[#0A66C2] border-b-2 border-[#0A66C2]" : "bg-transparent text-slate-400 hover:bg-slate-50"}`}>
                  <Icon name={tab.icon} className="w-3.5 h-3.5" />{tab.label}
                </button>
              ))}
            </div>

            {activeTab === "info" ? <InfoSection applicant={selected} /> : <DocumentsSection applicant={selected} />}
          ...
          <div className="flex-1 flex items-center justify-center bg-white border border-slate-200 rounded-2xl text-slate-400">
            <div className="text-center">
              <Icon name="users" className="w-8 h-8 text-slate-300 mx-auto mb-3" />
